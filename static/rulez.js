...
// Pass this if window is not defined yet
    ...
    var Rulez = function (config) {
        ...
        this.setGuideConfig = function (newGuides) {
            while (guides[0]) {
                removeGuide(guides[0], c.guides[0]);
            }
            c.guides = newGuides.map(function (guide) {
                return mergeConfigs(guide, c.guideDefaults);
            });
            generateGuides();
        };
        ...
        function _createGuideRect(guideConfig) {
            // var guide = _createRectGeneral(0, guideConfig.className, guideConfig.getSize(guideConfig), guideConfig.strokeWidth, 0, 3);
            var guide = _createTriaGeneral(0, guideConfig.className, guideConfig.getSize(guideConfig), guideConfig.strokeWidth, 0, 3);
            console.log(guide)
            moveGuide(guide, guideConfig);
            return guide;
        }
        ...
        function _createRectGeneral(pos, className, lineLength, strokeWidth, alignment, pad) {
            ...
            if (pad){
                var wrapper = window.document.createElementNS(svgNS, 'g');
                wrapper.appendChild(line);
                var paddingElement = line.cloneNode();
                paddingElement.style.opacity = 0;
                paddingElement.setAttribute(x, addUnits(pos - pad));
                paddingElement.setAttribute(width, addUnits((pad * 2) + strokeWidth));
                wrapper.appendChild(paddingElement);
                return wrapper;
            }
            return line;
        }

        function _createTriaGeneral(pos, className, lineLength, strokeWidth, alignment, pad) {
            var polygonDom = window.document.createElementNS(svgNS, 'polygon');
            var defaultAlignment = isDefaultAlignment();
            var x, y, height, width;
            if (isVertical()) {
                polygonDom.setAttribute('points','240,0 240,30 210,15');//侧
                height = 'width';
                width = 'height';
            } else {
                polygonDom.setAttribute('points','0,50 30,50 15,80');//上
                height = 'height';
                width = 'width';
            }
            
            var alignmentValue = (typeof alignment !== 'undefined') ? alignment : (defaultAlignment ? '0' : getAlignmentOffset() - lineLength);
            polygonDom.setAttribute('class', className);
            polygonDom.setAttribute('style','fill:black;strock:#ccc;stroke-width:1;')
            

            if (pad){
                var wrapper = window.document.createElementNS(svgNS, 'g');
                wrapper.appendChild(polygonDom);
                var paddingElement = polygonDom.cloneNode();
                paddingElement.style.opacity = 0;
                paddingElement.setAttribute(x, addUnits(pos - pad));
                paddingElement.setAttribute(width, addUnits((pad * 2) + strokeWidth));
                wrapper.appendChild(paddingElement);
